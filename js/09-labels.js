...
// QA Tag Manager Functions
function openQAManager() {
  openModal('modal-qa-manager');
  renderQAManager();
}

function renderQAManager() {
  const container = document.getElementById('qa-manager-list');
  if (!container) return;

  let html = '';
  QA_TAGS.forEach((cat, idx) => {
    html += `
      <div class="qa-manager-item" data-idx="${idx}" draggable="true">
        <div class="qa-manager-header">
          <span class="qa-drag-handle" draggable="false">☰</span>
          <input class="qa-cat-edit-input" value="${cat.cat}" data-idx="${idx}" data-field="cat" onchange="updateQACategory(${idx}, 'cat', this.value)">
          <select class="qa-manager-cls-select" onchange="updateQACategory(${idx}, 'cls', this.value)">
            <option value="cat-layout" ${cat.cls === 'cat-layout' ? 'selected' : ''}>布局 (蓝)</option>
            <option value="cat-size" ${cat.cls === 'cat-size' ? 'selected' : ''}>尺寸 (绿)</option>
            <option value="cat-color" ${cat.cls === 'cat-color' ? 'selected' : ''}>颜色 (紫)</option>
            <option value="cat-text" ${cat.cls === 'cat-text' ? 'selected' : ''}>内容 (黄)</option>
            <option value="cat-asset" ${cat.cls === 'cat-asset' ? 'selected' : ''}>样式 (粉)</option>
          </select>
          <div class="qa-manager-actions">
            <button class="qa-manager-btn danger" onclick="deleteQACategory(${idx})" title="删除分类">🗑️</button>
          </div>
        </div>
        <div class="qa-tags-edit">
          ${cat.tags.map((tag, tagIdx) => `
            <div class="qa-tag-edit-row">
              <input class="qa-tag-edit-input" value="${tag}" onchange="updateQATag(${idx}, ${tagIdx}, this.value)">
              <button class="qa-manager-btn danger" onclick="deleteQATag(${idx}, ${tagIdx})" title="删除标签">✕</button>
            </div>
          `).join('')}
          <div class="qa-add-tag-row">
            <input class="qa-add-tag-input" placeholder="添加新标签..." id="qa-add-input-${idx}" onkeydown="if(event.key==='Enter'){addQATag(${idx})}">
            <button class="qa-add-tag-btn" onclick="addQATag(${idx})">+ 添加</button>
          </div>
        </div>
      </div>
    `;
...
function updateQACategory(idx, field, value) {
  if (field === 'cat') {
    QA_TAGS[idx].cat = value;
  } else if (field === 'cls') {
    QA_TAGS[idx].cls = value;
  }
}

function updateQATag(catIdx, tagIdx, value) {
  QA_TAGS[catIdx].tags[tagIdx] = value;
}

function addQATag(catIdx) {
  const input = document.getElementById(`qa-add-input-${catIdx}`);
  if (!input || !input.value.trim()) return;

  const tagValue = input.value.trim();
  if (!QA_TAGS[catIdx].tags.includes(tagValue)) {
    QA_TAGS[catIdx].tags.push(tagValue);
    renderQAManager();
  }
  input.value = '';
}

function deleteQATag(catIdx, tagIdx) {
  QA_TAGS[catIdx].tags.splice(tagIdx, 1);
  renderQAManager();
}

function deleteQACategory(idx) {
  if (confirm('确定要删除这个分类及其所有标签吗？')) {
    QA_TAGS.splice(idx, 1);
    renderQAManager();
  }
}

function addQACategory() {
  const newCat = {
    cat: '新分类',
    cls: 'cat-layout',
    tags: ['新标签']
  ...
  setTimeout(() => {
    const newCatInput = document.querySelector(`.qa-manager-item[data-idx="${QA_TAGS.length - 1}"] .qa-cat-edit-input`);
    if (newCatInput) {
      newCatInput.select();
      newCatInput.focus();
    }
...
function saveQATags() {
  saveQATagsToStorage(QA_TAGS);
  renderQAPanel();
  updateQATagsState();
  closeModal('modal-qa-manager');
}

// Drag and drop functions for category reordering
...
function initDragAndDrop() {
  ...
  container.addEventListener('dragstart', (e) => {
    const item = e.target.closest('.qa-manager-item');
    if (!item) return;
  ...
  container.addEventListener('dragend', (e) => {
    if (draggedItem) {
      ...
      document.querySelectorAll('.qa-manager-item.drag-over').forEach(el => {
  ...
  container.addEventListener('drop', (e) => {
    e.preventDefault();
    const dropTarget = e.target.closest('.qa-manager-item');
    if (!dropTarget || !draggedItem || dropTarget === draggedItem) return;

    const targetIndex = parseInt(dropTarget.dataset.idx);
    if (isNaN(targetIndex) || draggedIndex === null) return;

    // Reorder the array
    const [removed] = QA_TAGS.splice(draggedIndex, 1);
    QA_TAGS.splice(targetIndex, 0, removed);
...
// Initialize drag and drop when the modal opens
const originalOpenQAManager = openQAManager;
openQAManager = function() {
  originalOpenQAManager();
  setTimeout(initDragAndDrop, 100); // Wait for DOM to be ready
};
...
function makeLabelDraggable(el, lb, mode){
  ...
  el.addEventListener('mousedown',e=>{
    ...
    e.preventDefault(); e.stopPropagation(); dragging=true; hasMoved=false; isDraggingLabel = false;
    startX=e.clientX; startY=e.clientY;
    if(mode==='label'){ startLx=lb.x; startLy=lb.y; }
    else{ startLx=lb.tx; startLy=lb.ty; }
    // Only select label and sync panels on first click, not during drag
  ...
  document.addEventListener('mouseup',()=>{
    ...
    // 仅在纯点击（无拖拽位移）时才打开编辑浮窗，拖拽时不触发
    if(!hasMoved){
      const existingCrop = document.getElementById('active-crop-float');
      if(!existingCrop || existingCrop.dataset.labelId !== lb.id){
        syncActiveCropFloat();
      }
    }
  });
...
function selectIssueFromDrawer(id) {
  // Don't re-select if already selected to avoid panel reopening
...
// Global flag to prevent panel reopening during drag
...
function syncActiveCropFloat() {
  ...
  // Don't recreate panel if dragging the same label or if it already exists for this label
  if(isDraggingLabel && existing && existing.dataset.labelId===selectedLabelId) return;
...
function _applyDiagIssues(issues) {
  issues.forEach((issue,idx)=>{
    ...
    canvasLabels.push({
      ...
      opacity:1, autoGenerated:true, diagDesc:issue.desc
    });
  });
  renumberLabels();
