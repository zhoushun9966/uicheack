...
function toggleCompare() {
  ...
  const modeBtns = ['mode-compare','mode-outline','mode-findiff'];
    ...
    setMode(currentMode);
    // 重新显示标注内容
    if(annoCanvas) annoCanvas.style.display = '';
    ...
    const of=document.getElementById('outline-filters'); if(of) of.style.display='none';
    ...
    if(annoCanvas){ annoCtx.clearRect(0,0,annoCanvas.width,annoCanvas.height); annoCanvas.style.display='none'; }
  }
}
...
function setMode(mode) {
  ...
  // 轮廓过滤器只在轮廓对比激活时显示
  const of = document.getElementById('outline-filters');
  if(of) of.style.display = (isActive && mode==='outline') ? 'flex' : 'none';
  updateEraserLabels();
  if(!isActive) return;
  renderCurrentMode();
}
...
function renderFindiff() {
  ...
  octx.clearRect(0,0,w,h); _drawPsCentered(octx,w,h); const d2=octx.getImageData(0,0,w,h).data;
...
// ====== PIXEL ANALYSIS ======
// 将设计稿按 "cover" 逻辑居中裁剪后绘制到目标画布，保证与游戏截图对齐
// - 设计稿更宽 → 裁两侧，高度填满
// - 设计稿更高 → 裁上下，宽度填满
function _drawPsCentered(ctx, w, h) {
  if (!psImg) return;
  if (!gameImg) { ctx.drawImage(psImg, 0, 0, w, h); return; }

  const psNW = psImg.naturalWidth, psNH = psImg.naturalHeight;
  const gameRatio = gameImg.naturalWidth / gameImg.naturalHeight;
  const psRatio   = psNW / psNH;

  let sx, sy, sw, sh;
  if (psRatio > gameRatio + 0.001) {
    // 设计稿更宽：以高度为基准，裁两侧
    sh = psNH;
    sw = Math.round(gameRatio * psNH);
    sx = Math.round((psNW - sw) / 2);
    sy = 0;
  } else if (psRatio < gameRatio - 0.001) {
    // 设计稿更高：以宽度为基准，裁上下（居中）
    sw = psNW;
    sh = Math.round(psNW / gameRatio);
    sx = 0;
    sy = Math.round((psNH - sh) / 2);
  } else {
    // 宽高比一致，全图缩放
    sx = 0; sy = 0; sw = psNW; sh = psNH;
  }

  ctx.drawImage(psImg, sx, sy, sw, sh, 0, 0, w, h);
}
...
function renderOutline(){
  ...
  if(!syncCanvasSize()) return;
  const w=markCanvas.width, h=markCanvas.height;
  const psCropped=document.createElement('canvas'); psCropped.width=w; psCropped.height=h;
  _drawPsCentered(psCropped.getContext('2d'),w,h);
  const edges1=detectEdges(gameImg,w,h);
  const edges2=detectEdges(psCropped,w,h);

  const showGame    = document.getElementById('ol-game')?.checked    ?? true;
  const showDesign  = document.getElementById('ol-design')?.checked  ?? true;
  const showOverlap = document.getElementById('ol-overlap')?.checked ?? true;
  ...
  for(let i=0;i<w*h;i++){
    ...
    let r=0, gv=0;
    if(g&&d){
      if(showOverlap){ r=255; gv=255; }
    } else if(g&&showGame){
      r=edges1[i];
    } else if(d&&showDesign){
      gv=edges2[i];
    }
    od[idx]=r; od[idx+1]=gv; od[idx+2]=0;
    od[idx+3]=(r||gv)?255:0;
  }
...
function onOutlineFilterChange(){
  if(isActive && currentMode==='outline') renderOutline();
}
