...
function highlightSidebarTool(type) {
  ...
  if(type){
    ...
    const btnId=(type==='e-mark'||type==='e-ps'||type==='e-both')?'tool-eraser':'tool-'+type;
...
function renderPropPanel() {
  ...
  if(panelTool&&panelTool.startsWith('e-')){
    h+='<div class="prop-section"><div class="prop-label">橡皮类型</div>';
    ...
    h+='</div>';
    h+='<div class="toggle-row" style="margin-top:4px">';
    h+=`<button class="tog-btn${activeTool==='e-both'?' active':''}" onclick="setEraserType('e-both')">抹提示+设计稿</button>`;
    h+='</div></div>';
...
function toolName(t){const m={label:'标注',brush:'画笔','e-mark':'抹提示','e-ps':'抹设稿','e-both':'抹提示+设计稿'};return m[t]||t;}
...
function activateEraser() {
  ...
  if(activeTool==='e-mark'||activeTool==='e-ps'||activeTool==='e-both'){
