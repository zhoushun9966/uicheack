...
let labelProps = { color:'#007AFF', shape:'rect', lw:2, opacity:1 };
...
// Load QA tags from localStorage with fallback to default
function loadQATags() {
  const saved = localStorage.getItem('qaTags');
  if (saved) {
    try {
      return JSON.parse(saved);
    } catch (e) {
      console.warn('Failed to parse saved QA tags:', e);
    }
  }
  return null; // Return null instead of default array to distinguish from saved data
}

// Save QA tags to localStorage
function saveQATagsToStorage(tags) {
  localStorage.setItem('qaTags', JSON.stringify(tags));
}

// Initialize QA tags from storage
function initQATags() {
  const savedTags = loadQATags();
  // Only override default tags if user has explicitly saved custom ones
  if (savedTags && savedTags.length > 0) {
    QA_TAGS.length = 0;
    QA_TAGS.push(...savedTags);
  }
  // If no saved tags, keep the default QA_TAGS array as defined above
}
